"""RepNet model definition, adapted from Google's official RepNet colab
notebook (google-research/google-research, repnet/repnet_colab.ipynb,
Apache 2.0 license), retrieved 2026-08-20.

This is a near-verbatim vendor of the notebook's model-definition cell,
stripped of Colab-only pieces (matplotlib visualization, webcam capture,
yt-dlp download, IPython display). The `ResnetPeriodEstimator.__init__`
layer construction order is intentionally left untouched -- the checkpoint
loader in `checkpoint_mapping.py` restores weights by zipping a fixed
mapping list against `model.weights` positionally, so any reordering here
would silently load the wrong weights into the wrong layers.

Runs in the isolated `.venv-tf` environment (see repo root
`Makefile`/PROGRESS.md Phase 10) -- deliberately has no dependency on the
main `hybrid` package (mediapipe/torch/pydantic aren't installed here).
"""
...
class ResnetPeriodEstimator(tf.keras.models.Model):
    ...
    def __init__(
        ...
        self.base_model = tf.keras.models.Model(
            inputs=base_model.input, outputs=base_model.get_layer(self.base_model_layer_name).output
        )
        ...
        self.temporal_bn_layers = [layers.BatchNormalization() for _ in self.temporal_conv_layers]

        # Counting Module (Self-sim > Conv > Transformer > Classifier)
        self.conv_3x3_layer = layers.Conv2D(
            self.conv_channels, self.conv_kernel_size, padding="same", activation=tf.nn.relu
        )

        channels = self.transformer_layers_config[0][0]
        self.input_projection = layers.Dense(
            channels, kernel_regularizer=regularizers.l2(self.l2_reg_weight), activation=None
        )
        ...
        for d_model, num_heads, dff in self.transformer_layers_config:
            self.transformer_layers.append(
                TransformerLayer(d_model, num_heads, dff, self.transformer_dropout_rate, self.transformer_reorder_ln)
            )
        ...
        for channels in self.period_fc_channels:
            self.fc_layers.append(
                layers.Dense(channels, kernel_regularizer=regularizers.l2(self.l2_reg_weight), activation=tf.nn.relu)
            )
        self.fc_layers.append(layers.Dense(num_preds, kernel_regularizer=regularizers.l2(self.l2_reg_weight)))
        ...
        self.within_period_fc_layers.append(
            layers.Dense(num_preds, kernel_regularizer=regularizers.l2(self.l2_reg_weight))
        )
    ...
    def call(self, x, training=False):
        ...
        for bn_layer, conv_layer in zip(self.temporal_bn_layers, self.temporal_conv_layers):
            x = conv_layer(x)
...
def get_repnet_model(logdir):
    """Returns a trained RepNet model (weights not yet checkpoint-mapped --
    see checkpoint_mapping.load_ckpt_with_custom_layer_mapping, required in
    addition to this for a correct load).
    """
    assert tf.executing_eagerly()

    model = ResnetPeriodEstimator()
    # Pass dummy frames to build graph.
    _ = model(tf.random.uniform((1, 64, 112, 112, 3)), training=False)
...
def get_score(period_score, within_period_score):
    ...
    pred_period_conf = tf.where(tf.math.less(per_frame_periods, 3), 0.0, pred_period_conf)
...
def get_counts(
    ...
    """Pass frames through model and convert period predictions to count."""
    ...
    for stride in strides:
        ...
        for batch_idx in range(num_batches):
            ...
            curr_frames = tf.reshape(curr_frames, [batch_size, model.num_frames, model.image_size, model.image_size, 3])

            raw_scores, within_period_scores, _ = model(curr_frames)
            raw_scores_per_stride.append(np.reshape(raw_scores.numpy(), [-1, raw_scores.shape[-1]]))
        ...
        pred_score, within_period_score_stride = get_score(raw_scores_per_stride, within_period_score_stride)
    ...
    raw_scores = np.repeat(raw_scores_list[argmax_strides], chosen_stride, axis=0)[:seq_len]
    within_period = np.repeat(within_period_scores_list[argmax_strides], chosen_stride, axis=0)[:seq_len]
    ...
    if constant_speed:
        scores = tf.reduce_mean(tf.nn.softmax(raw_scores[periodic_idxes], axis=-1), axis=0)
        max_period = np.argmax(scores)
        pred_score = scores[max_period]
        pred_period = chosen_stride * max_period
        per_frame_counts = np.asarray(seq_len * [1.0 / pred_period]) * np.asarray(within_period_binary)
    else:
