...
# Standard MediaPipe Hands 21-point skeleton topology -- fixed anatomy, not
# a tunable parameter, so a code constant rather than config (used by
# overlay_video.py to draw a connected hand skeleton, not just dots;
# mp.solutions is unavailable in this installed mediapipe build, so this
# can't be imported from the library itself).
