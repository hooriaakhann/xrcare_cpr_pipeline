...
## Phase 12 — Final Hybrid Fusion (done)

**`src/hybrid/fusion.py`:** `fuse_estimates()` combines the five candidate CPM estimates (CWT/autocorrelation/FFT/peaks from Phase 9, RepNet from Phase 10) into one `final_cpm` — deliberately not a plain confidence-weighted average, per spec's explicit "basic disagreement handling" requirement. Each candidate's final weight is `confidence_i * agreement_weight_i`, where `agreement_weight_i = 1 / (1 + |cpm_i - center| / disagreement_scale_cpm)` and `center` is the **confidence-weighted median** of the usable candidates (median chosen over mean specifically for its robustness to a single outlier, then confidence-weighted so higher-trust estimators have more say in the exact value). This directly implements spec's own worked example — a low-confidence outlier gets discounted *twice*: once by its own low confidence, once again by how far it sits from where the others agree — so it can't pull the final value strongly toward it even in the edge case where its raw confidence isn't dramatically low on its own.

**RepNet's `cpm=None` case** (Phase 10: "no periodicity trusted") is handled explicitly, not as an error: still recorded in the output `candidates` list (for full transparency, per spec's "store each estimate, each confidence, each fusion weight") with `final_weight=0.0`, simply excluded from the weighted average. `FusionError` is reserved for the genuinely-shouldn't-happen case where *no* candidate is usable at all (Phase 9's estimators already raise `EstimatorError` upstream rather than returning `None`, so in practice only RepNet can ever be missing — this exception exists for defensive completeness, exercised directly in a test via monkey-patched dataclass fields rather than a naturally-occurring scenario). An all-zero-confidence edge case (every candidate confidence is exactly 0) falls back to an unweighted mean of usable candidates rather than raising or dividing by zero — there's still data, just no trusted way to weight it.

**Stored per spec:** every candidate's estimate, confidence, agreement weight, and final weight (`CandidateEstimate`); the fusion center used for the disagreement calc; `final_cpm`; `overall_confidence` (`Σfinal_weight / 5`, so it's naturally lower when a candidate is missing or weighted to near-zero, not just when confidences are individually low); `spread_cpm` (max-min) and `std_cpm` — the "estimator disagreement/spread" spec asks for.

**Config (`fusion:`):** `disagreement_scale_cpm` (10.0) — how many CPM of deviation from center before an estimate's agreement weight meaningfully drops; a first-pass defensible default (Phase 15 tunes it against dev accuracy, not chosen in isolation here). Schema-validated (`> 0`).

**Tests (`tests/test_fusion.py`, 8 tests):** weighted-median correctness (uniform and skewed-weight cases), full-agreement/high-confidence sanity (final lands within 0.5 CPM of the unanimous value), **the spec's own worked example verbatim** (CWT~100/autocorr~101/FFT~100/peaks~102 vs. RepNet~65 at confidence 0.15 — asserts `final_cpm > 95` and RepNet's `final_weight < 0.1`), RepNet-`None` exclusion-but-still-recorded, the all-zero-confidence fallback, spread/std correctness, and `FusionError` when nothing is usable. Full suite: **181/184 tests passing** (3 slow deselected), ruff clean.

**Real-data check** (full Phase 2->12 chain, 2 dev videos): `video3_development.mp4` (GT 105.88) — **final_cpm=103.71 (err -2.17, ~2%)**, overall_confidence 0.574; RepNet (92.66, the most distant candidate from center=105.88) was correctly double-discounted to `final_weight=0.285` (confidence 0.662 x agreement 0.431) — the same disagreement-handling mechanism verified in the spec-example unit test, now confirmed against real fusion output, not just synthetic numbers. `video1_development.mp4` (GT 90.00) — **final_cpm=92.71 (err +2.71, ~3%)** — notably *better* than every individual Phase 9 classical estimator on this same video (which each overshot by +3.16 to +5.47 CPM alone); RepNet's lower estimate (85.16) pulling the weighted average back toward GT is a first real (if anecdotal, single-video) sign the hybrid can outperform its individual branches, exactly the question Phase 14's ablation is designed to answer rigorously across the whole dev set.
